...
//	@Scheduled(cron= "0 * */1 * * ?")     ////0 * */1 * * ?:每两个小时执行一次；                    0 0/1 * * * ? ：每分钟执行一次
	public void fensAllTeamTempClob(){
        ...
        if(allList.size() > 0){
        	for(int j = 0; j < allList.size(); j++){
        		...
        		fuInfo.setFensCount(AFensTeamNum.longValue());// 粉丝团用户数量
...
//	@Scheduled(cron= "0 10/1 * * * ?")     ////0 * * /2 * * ?:每两个小时执行一次；                    0 0/1 * * * ? ：每分钟执行一次
