...
import com.arttraining.api.beanv2.RechargeModel;
...
public class MobilePayController {
	...
	@RequestMapping(value = "/payment/system", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
	@ResponseBody
	public String paymentSystem(HttpServletRequest request, HttpServletResponse response, MessageModel model) {
		...
		if (order_id == null || pay_method == null || pay_source == null
			...
			String product_name="支付云互艺商品";//订单名称
			String order_price="";//订单金额(ali)
			String order_price_wx="";//订单金额(wx)
			try {
				WalletSystem order = this.paymentSystemService.getWalletSystemInfoByOrderNumber(order_id);
	...
	@ResponseBody
	public String recharge(HttpServletRequest request, HttpServletResponse response, MessageModel model) {
		String accessToken = request.getParameter("access_token");
		String uid = request.getParameter("uid");
		String order_id = request.getParameter("order_number");
		String pay_method = request.getParameter("pay_method");//支付方式：wxpay||alipay
		...
		RechargeModel re=new RechargeModel();
		...
		if (order_id == null || pay_method == null || pay_source == null
			...
			String product_name="充值云币";//订单名称
			String order_price="";//订单金额(ali)
			String order_price_wx="";//订单金额(wx)
			try {
				WalletOrder order=this.walletService.getOrderInfoByNumber(order_id);
				order_price = order.getFinalPay().toString();
				int int_order_price = (int)(Double.parseDouble(order_price)*100);//微信金额 以分为单位
				order_price_wx =  Integer.toString(int_order_price);
			} catch (Exception e) {
//				model.setError_code("20032");
//				model.setError_msg(ErrorCodeConfigUtil.ERROR_MSG_ZH_20032);
				re.setError_code("20032");
				re.setError_msg(ErrorCodeConfigUtil.ERROR_MSG_ZH_20032);
				e.printStackTrace();
			}
			if((PAY_METHOD_WX).equals(pay_method)){
				// 初始化 RequestHandler
...
//			model.setError_code("20028");
//			model.setError_msg(ErrorCodeConfigUtil.ERROR_MSG_ZH_20028);
			re.setError_code("20028");
			re.setError_msg(ErrorCodeConfigUtil.ERROR_MSG_ZH_20028);
		}
...
//		uid = "0";
		...
		if (order_id == null || pay_method == null || pay_source == null
			...
			try {
				...
				order_price = NumberUtil.formatDouble1(order.getFinalPay()).toString();
