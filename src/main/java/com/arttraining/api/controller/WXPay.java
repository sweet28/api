...
public class WXPay {
	...
	public void wxpay(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if ((sign).equals(map.get("sign"))) {
				...
				// 查询数据库操作
				Order order = this.ordersService
						.selectByOrderNumber(order_number);
				String order_price = order.getFinalPay().toString();
				// String order_price = "0.1";//demo，数据库中保存价格1元
	...
	// String returnValue =
	// "<xml><return_code><![CDATA[STATUS]]></return_code><return_msg><![CDATA[MESSAGE]]></return_msg></xml>";
	// try {
	// Map<String, Object> params = new HashMap<String, Object>();
	// Map<String, Object> pay = new HashMap<String, Object>();
