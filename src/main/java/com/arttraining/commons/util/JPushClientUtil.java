...
//	        }
//		}
		public static String eclose_push_extra_json_data(String type,String value) {
		...
		//封装Map对象--推送别名的消息方法
		public static void enclose_push_data_alias(String user_type,String push_type,
				String alias,String alert, String content,
				String content_type,String extra_value) {
			...
			push_all_alias_alert_msg(push_map);
		}
		
		//进行推送的关键在于构建一个 PushPayload对象
		//方法一:推送自定义消息msg和用户手机消息框内容
		public static void push_all_alias_alert_msg(Map<String, Object> map) {
			//用户类型
			String user_type=(String)map.get("user_type");
			if(user_type.equals("stu")) {
				jpush = new JPushClient(AHZ_MASTERSECRET,AHZ_APPKEY,3);
			} else if(user_type.equals("ms")) {
				jpush = new JPushClient(MS_MASTERSECRET,MS_APPKEY,3);
			} else if(user_type.equals("dr")) {
				jpush = new JPushClient(DR_MASTERSECRET,DR_APPKEY,3);
			}
	        ...
	        if(push_type.equals("alert")) {
	        	payload=buildPushObject_all_alias_alert(map);
	        } else if(push_type.equals("msg")) {
	        	payload=buildPushObject_all_alias_msg(map);
	        } else if(push_type.equals("alert_msg")) {
	        	payload=buildPushObject_all_alias_alert_msg(map);
	        }
	        try {
	            PushResult result = jpush.sendPush(payload);
	            System.out.println("Got result - " + result);
		 ...
		 * @return
		 */
		//1.推送自定义消息msg和用户手机消息框内容
		public static PushPayload buildPushObject_all_alert_msg(Map<String, Object> map) { 
					...
					.setNotification(Notification.newBuilder()
									...
									.addPlatformNotification(
											AndroidNotification.newBuilder()
													.setTitle(TITLE)
													.addExtra(ALERT_KEY, extra_value)
													.build())
									.addPlatformNotification(
	   ...
	   //2.只推送自定义消息msg
		public static PushPayload buildPushObject_all_msg(Map<String, Object> map) { 
			String extra_value=(String)map.get("extra_value");
			String content=(String)map.get("content");
			String content_type=(String)map.get("content_type");
		...
		//3.所有平台,所有设备,只推送内容为 ALERT 的通知
		public static PushPayload buildPushObject_all_all_alert(Map<String, Object> map) {  
			String alert=(String)map.get("alert");
			return PushPayload.alertAll(alert);  
		}  
		//4.向指定的用户推送相应的消息:别名 默认是用户ID
		public static PushPayload buildPushObject_all_alias_alert_msg(Map<String, Object> map) { 
	   ...
	   //5.向指定的别名用户只推送自定义消息
		public static PushPayload buildPushObject_all_alias_msg(Map<String, Object> map) { 
			String alias=(String)map.get("alias");
			String extra_value=(String)map.get("extra_value");
			String content=(String)map.get("content");
			String content_type=(String)map.get("content_type");
	   ...
	   //6.向指定的别名用户只推送内容为 ALERT的通知
		public static PushPayload buildPushObject_all_alias_alert(Map<String, Object> map) {
			String alias=(String)map.get("alias");
			String alert=(String)map.get("alert");
			String extra_value=(String)map.get("extra_value");
